	...
	BNInstructionTextToken** result, size_t* resultCount)
{
	...
	vector<InstructionTextToken> tokens = printer->GetTypeTokens(
		new Type(BNNewTypeReference(type)), new Platform(BNNewPlatformReference(platform)),
	...
	size_t* resultCount)
{
	...
	vector<InstructionTextToken> tokens = printer->GetTypeTokensBeforeName(
		new Type(BNNewTypeReference(type)), new Platform(BNNewPlatformReference(platform)),
		parentPointer, baseConfidence, escaping);
...
{
	vector<InstructionTextToken> before = GetTypeTokensBeforeName(type, platform, baseConfidence, false, escaping);
	vector<InstructionTextToken> after = GetTypeTokensAfterName(type, platform, baseConfidence, false, escaping);
...
{
	vector<InstructionTextToken> tokens = GetTypeTokensBeforeName(type, platform, BN_FULL_CONFIDENCE, false, escaping);
	string result;
...
{
	vector<InstructionTextToken> tokens = GetTypeTokensAfterName(type, platform, BN_FULL_CONFIDENCE, false, escaping);
	string result;
	for (const auto& i : tokens)
		result += i.text;
	return result;
	...
	uint8_t baseConfidence, BNTokenEscapingType escaping)
{
	...
	bool success = BNGetTypePrinterTypeTokens(GetObject(), type->GetObject(),
		platform->GetObject(), &qname, baseConfidence, escaping, &tokens, &tokenCount);
...
std::vector<InstructionTextToken> CoreTypePrinter::GetTypeTokensBeforeName(Ref<Type> type,
	...
	bool parentPointer, BNTokenEscapingType escaping)
{
	BNInstructionTextToken* tokens;
	size_t tokenCount;

	bool success = BNGetTypePrinterTypeTokensBeforeName(GetObject(), type->GetObject(), platform->GetObject(), baseConfidence, parentPointer, escaping, &tokens, &tokenCount);
	...
	bool parentPointer, BNTokenEscapingType escaping)
{
	...
	bool success = BNGetTypePrinterTypeTokensAfterName(GetObject(), type->GetObject(), platform->GetObject(), baseConfidence, parentPointer, escaping, &tokens, &tokenCount);
	...
	const QualifiedName& name, BNTokenEscapingType escaping)
{
	...
	char* result;
	bool success = BNGetTypePrinterTypeString(GetObject(), type->GetObject(), platform->GetObject(), &qname, escaping, &result);
...
{
	char* result;
	bool success = BNGetTypePrinterTypeStringBeforeName(GetObject(), type->GetObject(), platform->GetObject(), escaping, &result);
...
{
	char* result;
	bool success = BNGetTypePrinterTypeStringAfterName(GetObject(), type->GetObject(), platform->GetObject(), escaping, &result);
