...
)
{
	...
	bool success = parser->ParseTypesFromSource(new Platform(platform), source, fileName,
	...
	result->types = new BNQualifiedNameAndType[resultCpp.types.size()];
	result->variables = new BNQualifiedNameAndType[resultCpp.variables.size()];
	result->functions = new BNQualifiedNameAndType[resultCpp.functions.size()];
...
)
{
	...
	vector<TypeParserError> errorsCpp;
	bool success = parser->ParseTypeString(new Platform(platform), source, existingTypesCpp,
		resultCpp, errorsCpp);
...
bool CoreTypeParser::ParseTypesFromSource(Ref<Platform> platform, const std::string& source,
	const std::string& fileName, const std::map<QualifiedName, TypeAndId>& existingTypes,
	...
	const std::string& autoTypeSource, TypeParserResult& result, std::vector<TypeParserError>& errors)
{
	...
	auto success = BNTypeParserParseTypesFromSource(m_object, platform->GetObject(), source.c_str(), fileName.c_str(),
		apiExistingTypes, existingTypes.size(),
...
bool CoreTypeParser::ParseTypeString(Ref<Platform> platform, const std::string& source,
	...
	QualifiedNameAndType& result, std::vector<TypeParserError>& errors)
{
	...
	auto success = BNTypeParserParseTypeString(m_object, platform->GetObject(), source.c_str(),
